...
from appy.db.indexer import XhtmlTextExtractor
...
class String(Field):
    ...
    pxCell = Px('''
     <x if="field.isUrl" var2="value=zobj.getFieldValue(name)">
    ...
    pxFilterSelect = Px('''
     <select var="name=field.name;
      ...
      <option for="v in field.getPossibleValues(ztool, withTranslations=True,\
       ...
       value=":v[0]" selected=":(name in filters) and (filters[name] == v[0])"
       title=":v[1]">:ztool.truncateValue(v[1], charsWidth)</option>
     </select>''')
    ...
    def __init__(self, validator=None, multiplicity=(0,1), default=None,
      ...
      scolspan=1, swidth=None, sheight=None, persist=True, transform='none',
      placeholder=None, styles=('p','h1','h2','h3','h4'), allowImageUpload=True,
      spellcheck=False, languages=('en',), languagesLayouts=None,
      inlineEdit=False, toolbar='Standard', view=None, cell=None, xml=None,
      noValueLabel='choose_a_value'):
    ...
    def valueIsInRequest(self, obj, request, name, layoutType):
        ...
        return '%s_%s' % (name, languages[0]) in request
    ...
    def extractText(self, value, lower=True):
        '''Extracts pure text from XHTML p_value'''
        extractor = XhtmlTextExtractor(lower=lower, raiseOnError=False)
    ...
    def getPossibleValues(self, obj, withTranslations=False,
        ...
        '''Returns the list of possible values for this field (only for fields
           ...
           p_className, we will need to use obj.executeQuery to find one.'''
